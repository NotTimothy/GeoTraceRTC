...
async fn parse_json(json_data: Json<Value>) -> Result<WebRTCData, String> {
    let stats = json_data["PeerConnections"]["29-2"]["stats"]
        .as_object()
    ...
    for (key, value) in stats {
        if key.ends_with("-ip")
            || key.ends_with("-address")
            || key.ends_with("-port")
            || key.ends_with("-networkType")
            || key.ends_with("-protocol")
            || key.ends_with("-candidateType")
        {
...
async fn main() {
    let app = Router::new()
        .route("/api/process-file", post(process_json));
